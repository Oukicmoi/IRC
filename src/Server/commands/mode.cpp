...
void Server::cmd_MODE(User* user, IRCMessage& msg)
{
    ...
    if (params[0].empty() || (params[0][0] != '#'))
        return sendWhenReady(user->getSocketFd(), ERR_UMODEUNKNOWNFLAG(user->getNick()));

    Channel* channel = getChannelByName(params[0]);
    if (!channel)
        return sendWhenReady(user->getSocketFd(), ERR_NOSUCHCHANNEL(user->getNick(), params[0]));

    if (params.size() == 1)
        return channel->sendModesToUser(user);

    if (!channel->isOperator(user))
        return sendWhenReady(user->getSocketFd(), ERR_CHANOPRIVSNEEDED(user->getNick(), params[0]));
